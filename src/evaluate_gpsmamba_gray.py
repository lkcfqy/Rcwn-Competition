...
GPS_ROOT = ROOT / "external_models" / "GPSMamba"
...
def _load_gpsmamba_class():
    # The repo-level basicsr package imports optional metric utilities at import
    # time. For inference we only need registry.py and arch_util.py, so load a
    # minimal package namespace to keep the architecture import self-contained.
    for key in list(sys.modules):
        if key == "basicsr" or key.startswith("basicsr."):
            del sys.modules[key]

    basicsr_pkg = types.ModuleType("basicsr")
    basicsr_pkg.__path__ = [str(GPS_ROOT / "basicsr")]
    sys.modules["basicsr"] = basicsr_pkg

    utils_pkg = types.ModuleType("basicsr.utils")
    utils_pkg.__path__ = [str(GPS_ROOT / "basicsr" / "utils")]
    utils_pkg.get_root_logger = lambda *args, **kwargs: logging.getLogger("gpsmamba")
    sys.modules["basicsr.utils"] = utils_pkg

    archs_pkg = types.ModuleType("basicsr.archs")
    archs_pkg.__path__ = [str(GPS_ROOT / "basicsr" / "archs")]
    sys.modules["basicsr.archs"] = archs_pkg

    _load_module("basicsr.utils.registry", GPS_ROOT / "basicsr" / "utils" / "registry.py")
    _load_module("basicsr.archs.arch_util", GPS_ROOT / "basicsr" / "archs" / "arch_util.py")
    arch = _load_module("basicsr.archs.gpsmamba_arch", GPS_ROOT / "basicsr" / "archs" / "gpsmamba_arch.py")
    return arch.GPSMamba


GPSMamba = _load_gpsmamba_class()
...
def build_gpsmamba(scale: int) -> torch.nn.Module:
    return GPSMamba(
        upscale=scale,
        in_chans=3,
        img_size=64,
...
def forward_gray_tiled(
    ...
    if lr_gray.shape[0] != 1:
        raise ValueError("Tiled GPSMamba inference currently expects batch size 1.")
...
def validate(args) -> None:
    ...
    model = build_gpsmamba(args.scale).to(device).eval()
    ckpt = torch.load(args.weights, map_location="cpu")
    model.load_state_dict(normalize_state(ckpt, args.param_key), strict=True)
    n_params = sum(p.numel() for p in model.parameters()) / 1e6
    print(f"params={n_params:.2f}M preset=gpsmamba_x{args.scale}")
