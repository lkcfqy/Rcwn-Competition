...
def forward_gray(model: torch.nn.Module, lr_gray: torch.Tensor, gray_mode: str, tta: bool = False) -> torch.Tensor:
    if not tta:
...
def validate(args) -> None:
    ...
    model = build_pft(args.variant, args.scale, args.use_checkpoint).to(device).eval()
    ...
    for name in tqdm(names, desc="val", leave=False):
        ...
        with autocast_context(device, args.amp):
            pred = forward_gray(model, lr, args.gray_mode, tta=args.tta)
...
def parse_args():
    ...
    parser.add_argument("--variant", choices=["pft", "light"], default="pft")
