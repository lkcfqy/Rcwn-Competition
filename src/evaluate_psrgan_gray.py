...
PSRGAN_ROOT = ROOT / "external_models" / "Infrared_Image_SR_PSRGAN"
sys.path.insert(0, str(PSRGAN_ROOT))
for key in list(sys.modules):
    if key == "models" or key.startswith("models."):
        del sys.modules[key]
models_pkg = types.ModuleType("models")
models_pkg.__path__ = [str(PSRGAN_ROOT / "models")]
sys.modules["models"] = models_pkg
from models.network_msrresnet import D_msrresnet0  # noqa: E402
...
def forward_gray(model: torch.nn.Module, lr_gray: torch.Tensor, gray_mode: str, tta: bool) -> torch.Tensor:
    ...
    preds = []
    for mode in range(8):
        pred = forward_gray_once(model, _aug(lr_gray, mode), gray_mode)
...
def validate(args) -> None:
    ...
    model = D_msrresnet0(in_nc=3, out_nc=3, nc=64, nb=16, upscale=args.scale).to(device).eval()
    ckpt = torch.load(args.weights, map_location="cpu")
    model.load_state_dict(normalize_state(ckpt), strict=True)
    n_params = sum(p.numel() for p in model.parameters()) / 1e6
    print(f"params={n_params:.2f}M preset=psrgan_x{args.scale}")
