...
def save_hat_checkpoint(
    ...
    torch.save({"kind": "hat_gray", "state_dict": state, "config": config, "metrics": metrics or {}}, path)
...
@torch.no_grad()
def validate(model, root_dir, names, scale, device, amp, gray_mode, lpips_net="alex", native_io: bool = False):
    model.eval()
    ...
    for name in tqdm(names, desc="val", leave=False):
        ...
        with autocast_context(device, amp):
            pred = forward_gray(model, lr, scale, gray_mode, native_io=native_io)
...
def train(args):
    ...
    model = build_hat(args.variant, args.scale, args.use_checkpoint, native_io=args.native_io).to(device)
    ...
    if not args.no_initial_val:
        ...
        best = metrics["proxy"]
        save_hat_checkpoint(os.path.join(args.out_dir, f"best_x{args.scale}.pth"), model, cfg, metrics, half=False)
        save_hat_checkpoint(os.path.join(args.out_dir, f"best_x{args.scale}_fp16.pth"), model, cfg, metrics, half=True)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % args.val_every == 0 or epoch == args.epochs:
            ...
            final_proxy = metrics["proxy"]
            save_hat_checkpoint(os.path.join(args.out_dir, f"latest_x{args.scale}.pth"), model, cfg, metrics, half=False)
...
def parse_args():
    ...
    parser.add_argument("--out-dir", default="checkpoints_hat_gray")
