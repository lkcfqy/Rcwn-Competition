...
CRAFT_ROOT = ROOT / "external_models" / "CRAFT-SR"
...
def _load_craft_arch():
    basicsr = types.ModuleType("basicsr")
    basicsr.__path__ = [str(CRAFT_ROOT / "basicsr")]
    sys.modules["basicsr"] = basicsr

    archs = types.ModuleType("basicsr.archs")
    archs.__path__ = [str(CRAFT_ROOT / "basicsr" / "archs")]
    ...
    def get_root_logger(*args, **kwargs):
        ...
        return logging.getLogger("craft_eval")
    ...
    def modulated_deform_conv(*args, **kwargs):
        raise RuntimeError("CRAFT evaluation does not use BasicSR DCN ops.")

    dcn.modulated_deform_conv = modulated_deform_conv
    sys.modules["basicsr.ops.dcn"] = dcn

    spec = importlib.util.spec_from_file_location(
        "basicsr.archs.craft_arch",
        CRAFT_ROOT / "basicsr" / "archs" / "craft_arch.py",
    )
    if spec is None or spec.loader is None:
        raise ImportError(f"Could not load CRAFT from {CRAFT_ROOT}")
    module = importlib.util.module_from_spec(spec)
    sys.modules["basicsr.archs.craft_arch"] = module
    spec.loader.exec_module(module)
    return module.CRAFT


CRAFT = _load_craft_arch()
...
def build_craft(scale: int) -> torch.nn.Module:
    return CRAFT(
        upscale=scale,
        in_chans=3,
        img_size=64,
