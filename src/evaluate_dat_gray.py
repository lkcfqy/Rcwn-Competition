...
DAT_ROOT = ROOT / "external_models" / "DAT"
sys.path.insert(0, str(DAT_ROOT))
from basicsr.archs.dat_arch import DAT  # noqa: E402
...
def build_dat(variant: str, scale: int, use_checkpoint: bool) -> torch.nn.Module:
    if variant == "dat":
        ...
        raise ValueError(f"Unsupported DAT variant: {variant}")
    return DAT(
        upscale=scale,
        in_chans=3,
        img_size=64,
...
def parse_args():
    ...
    parser.add_argument("--variant", choices=["dat", "dat2", "dats"], default="dat")
