...
MAIR_ROOT = ROOT / "external_models" / "MaIR"
sys.path.insert(0, str(MAIR_ROOT))

from basicsr.archs.mair_arch import MaIR  # noqa: E402
...
def build_mair(variant: str, scale: int, use_checkpoint: bool) -> torch.nn.Module:
    if variant == "tiny":
        return MaIR(
            upscale=scale,
            in_chans=3,
    ...
    raise ValueError(f"Unsupported MaIR variant: {variant}")
...
def validate(args: Any) -> None:
    ...
    model = build_mair(args.variant, args.scale, args.use_checkpoint).to(device).eval()
...
def parse_args() -> Any:
    ...
    parser.add_argument("--variant", choices=["tiny", "small", "base"], default="base")
