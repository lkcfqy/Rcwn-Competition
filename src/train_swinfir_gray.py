...
from evaluate_swinfir_gray import build_model, forward_gray, normalize_state
...
def save_checkpoint(
    ...
    torch.save({"kind": "swinfir_gray", "state_dict": state, "config": config, "metrics": metrics or {}}, path)


@torch.no_grad()
def validate(model, window_size, root_dir, names, scale, device, amp, gray_mode, lpips_net="alex"):
    model.eval()
    ...
    for name in tqdm(names, desc="val", leave=False):
        ...
        with autocast_context(device, amp):
            pred = forward_gray(model, lr, scale, window_size, gray_mode)
...
def parse_args():
    ...
    parser.add_argument("--variant", choices=["swinfir", "swinfir_t", "hatfir", "hatfir_l"], default="hatfir")
    ...
    parser.add_argument("--degrade-sigma", type=float, default=0.7)
    parser.add_argument("--degrade-interp", choices=["area", "linear", "cubic", "lanczos"], default="cubic")
    ...
    parser.add_argument("--out-dir", default="checkpoints_swinfir_gray")
